...
const KDPunits=({match})=>{
 ...
 let [lgaList, setLgaList] = useState('')
    ...
    /*    const intervalId = setInterval(()=>{
        ...
        setLoading(true)
        axios.get(`https://bauchiem.herokuapp.com/api/v1//baupunits/getalllgpu/${type}/${selectedLGA}`)
        .then(res=>{
            setPunits(res.data)
            setLoading(false)
...
useEffect(()=>{
  ...
  //setTime(new Date())
    ...
    <h2 className={classes.cardTitle}>{`Time: ${time}`}
   { /*
    `Time: ${time} APC = ${totAPC} \n PDP = ${totPDP} Others = ${totOthers}`
    */
    }</h2>
                        </CardHeader>
                    </Card>
