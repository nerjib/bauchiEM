...
const Supervisorss=({match})=>{
   ...
   // const [tableData, setReport] = useState([]),
    ...
    useEffect(()=>{
        ...
        setLoading(true)
        axios.get(`https://kdpall.herokuapp.com/api/v1/incidents/supervisors`)
        .then(res=>{
            setPunits(res.data)
            setLoading(false)
...
useEffect(()=>{
    ...
    setLoading(true)
    axios.get(`https://kdpall.herokuapp.com/api/v1/incidents/ssupervisors`)
    .then(res=>{
        setPunits(res.data)
        setLoading(false)
  ...
  //setTime(new Date())
    ...
    if(punits.length>0){
        ...
        Object.keys(punits).map((e,i)=>
            {
                kk++
            punitData.push([ kk,punits[e].puid<10?'00'+punits[e].puid:'0'+punits[e].puid,punits[e].puname,punits[e].ward,punits[e].sender
            ])
